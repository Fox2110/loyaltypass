...
export default function App() {
  ...
  if (screen === "customer" && cust) {
    ...
    const unlockedBonuses = settings.rewards.filter(r => (cust.collectedBonuses||[]).some(b => b.rewardId === r.id) && !(cust.hiddenFromQR||[]).includes(r.id));
    const availCoupons = settings.coupons.filter(c => c.active && !(cust.redeemedCoupons||[]).includes(c.id) && !(cust.hiddenFromQR||[]).includes(c.id));
    const hiddenBonuses = settings.rewards.filter(r => (cust.collectedBonuses||[]).some(b => b.rewardId === r.id) && (cust.hiddenFromQR||[]).includes(r.id));
    ...
    const renderTab = () => {
      if (activeTab === "card") return (
        <div style={{ padding: "16px 16px 0" }}>
          ...
          <StampCard stamps={cust.stamps} settings={settings} onStamp={handleStamp} />
        ...
        return (
          <div style={{ padding: "16px 16px 0" }}>
            ...
            <StampCard stamps={c.stamps} settings={settings} onStamp={() => {
              ...
              setCustomers(cs => cs.map(cu => cu.id === c.id ? {
                ...cu, points: cu.points + pts, stamps: cu.stamps + 1, tier: calcTier(cu.points + pts),
                history: [{ id: genId(), date: today(), type: "earn", amount: pts, desc: `+${pts}P (Stempel)`, emp: "Admin" }, ...cu.history]
              } : cu));
            }} />
