...
const INIT_CUSTOMERS = [
  { id: "c1", username: "Max Mustermann", password: "welcome123", firstLogin: false, name: "Max Mustermann", birthdate: "1990-05-12", email: "max@email.de", phone: "+49 170 1234567", points: 340, stamps: 7, memberSince: "2024-01-15", tier: "gold", language: "de", darkMode: true, redeemedBonuses: [], redeemedCoupons: [], hiddenFromQR: [], history: [{ id: genId(), date: "2024-06-01", type: "earn", amount: 50, desc: "Einkauf 50€", emp: "Sophie" }, { id: genId(), date: "2024-06-10", type: "redeem", amount: -20, desc: "Punkte eingelöst", emp: "Tom" }] },
  { id: "c2", username: "Anna Schmidt", password: "welcome123", firstLogin: false, name: "Anna Schmidt", birthdate: "1995-08-23", email: "anna@email.de", phone: "+49 151 9876543", points: 120, stamps: 3, memberSince: "2024-03-10", tier: "silver", language: "de", darkMode: true, redeemedBonuses: [], redeemedCoupons: [], hiddenFromQR: [], history: [{ id: genId(), date: "2024-06-05", type: "earn", amount: 30, desc: "Einkauf 30€", emp: "Tom" }] },
];
...
function QRScannerCamera({ customers, settings, onScanned, onClose, t }) {
  ...
  const startCamera = async () => {
    try {
      ...
      setCameraError("Kamerazugriff verweigert");
    }
  };
...
function BonusRedeemPanel({ customer, settings, onRedeem, onClose, t }) {
  ...
  const pendingBonuses = settings.rewards.filter(r => {
    const alreadyRedeemed = !r.keepAfterRedeem && (customer.redeemedBonuses || []).includes(r.id);
    return prog >= r.stamps && !alreadyRedeemed;
  });
...
// ─── STAMP CARD ───────────────────────────────────────────────────────────────
function StampCard({ stamps, settings }) {
  const { stampCardMode, stampCardSize, rewards } = settings;
  const progress = stampCardMode === "loop" ? stamps % stampCardSize : stamps;
  ...
  return (
    <div style={{ ...styles.card, marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "var(--fg)" }}>Stempelkarte</div>
        ...
        {cells.map(i => {
          ...
          return (
            <div key={i} style={{
              ...
              background: filled
                ? "var(--accent)"
                : rw ? "var(--surface2)" : "var(--surface2)",
              border: `1.5px solid ${filled ? "var(--accent)" : rw ? "var(--accent)" : "var(--border)"}`,
              display: "flex", alignItems: "center", justifyContent: "center",
              flexDirection: "column", gap: 2,
              color: filled ? "#fff" : rw ? "var(--accent)" : "var(--fg3)",
              position: "relative",
              fontWeight: filled ? 700 : 500,
            }}>
              {filled
                ? (rw
                    ? <Icon v={rw.icon} size={18} style={{ color: "#fff" }} />
                    : <i className="fa-solid fa-check" style={{ fontSize: 16, color: "#fff" }}></i>)
                : (rw
                    ? <Icon v={rw.icon} size={18} />
                    : <span style={{ fontSize: 13, lineHeight: 1 }}>{i}</span>)
              }
              {rw && !filled && (
                <span style={{ fontSize: 8, fontWeight: 700, color: "var(--accent)", letterSpacing: 0.3, lineHeight: 1 }}>
                  {i}✦
                </span>
              )}
              {rw && <div style={{ position: "absolute", top: -4, right: -4, width: 9, height: 9, background: "#f59e0b", borderRadius: "50%", border: "2px solid var(--bg)" }} />}
            </div>
          );
        })}
      </div>
...
export default function App() {
  ...
  const redeemItem = (customerId, type, itemId, empId) => {
    setCustomers(cs => cs.map(c => {
      ...
      if (type === "bonus") {
        ...
        const newRedeemed = rw?.keepAfterRedeem ? c.redeemedBonuses : [...(c.redeemedBonuses||[]), itemId];
        return { ...c, redeemedBonuses: newRedeemed, points: c.points + bonusPoints,
          history: [{ id: genId(), date: today(), type: "redeem", amount: bonusPoints, desc: `Bonus eingelöst: ${rw?.name||itemId}`, emp: empId ? (employees.find(e=>e.id===empId)?.name||"") : "Admin" }, ...c.history] };
      } else {
        ...
        return { ...c, redeemedCoupons: newRedeemed, points: c.points + bonusPoints,
          history: [{ id: genId(), date: today(), type: "redeem", amount: bonusPoints, desc: `Coupon eingelöst: ${cp?.code||itemId}`, emp: empId ? (employees.find(e=>e.id===empId)?.name||"") : "Admin" }, ...c.history] };
      }
    }));
    if (type === "coupon") {
  ...
  if (screen === "customer" && cust) {
    ...
    const prog = settings.stampCardMode === "loop" ? cust.stamps % settings.stampCardSize : cust.stamps;
    ...
    // Build QR content: base + unlocked bonuses + coupons (minus hidden ones)
    const unlockedBonuses = settings.rewards.filter(r => prog >= r.stamps && !(cust.redeemedBonuses||[]).includes(r.id) && !(cust.hiddenFromQR||[]).includes(r.id));
    const availCoupons = settings.coupons.filter(c => c.active && !(cust.redeemedCoupons||[]).includes(c.id) && !(cust.hiddenFromQR||[]).includes(c.id));
    ...
    // All items that are hidden but not yet redeemed (so user can re-add them)
    const hiddenBonuses = settings.rewards.filter(r => prog >= r.stamps && !(cust.redeemedBonuses||[]).includes(r.id) && (cust.hiddenFromQR||[]).includes(r.id));
    const hiddenCoupons = settings.coupons.filter(c => c.active && !(cust.redeemedCoupons||[]).includes(c.id) && (cust.hiddenFromQR||[]).includes(c.id));
      ...
      if (activeTab === "bonus") return (
        <div style={{ padding: "16px 16px 0" }}>
          ...
          {settings.rewards.map(r => {
            ...
            const redeemed = (cust.redeemedBonuses||[]).includes(r.id) && !r.keepAfterRedeem;
            return (
              <div key={r.id} style={{ ...styles.card, marginBottom: 10, opacity: redeemed ? 0.5 : 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ fontSize: 22, width: 44, height: 44, background: "var(--surface2)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon v={r.icon} size={20} /></div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "var(--fg)" }}>{r.name}</div>
                  ...
                  {redeemed ? <span style={styles.badge("blue")}>Eingelöst</span>
                    : available ? <span style={styles.badge("green")}><i className="fa-solid fa-check"></i></span>
    ...
    return (
      <div style={{ ...wrapStyle, ...theme }}>
        ...
        {modal === "points" && scannedCustomer && (
          <Modal onClose={() => { setModal(null); setScannedCustomer(null); }}>
            <PointSlider customer={scannedCustomer} euroPerPoint={settings.euroPerPoint} onConfirm={pts => { addPoints(scannedCustomer.id, pts, activeUser.id); setModal(null); setScannedCustomer(null); }} onClose={() => { setModal(null); setScannedCustomer(null); }} t={lt} />
          </Modal>
        )}
      ...
      if (selectedCust) {
        ...
        return (
          <div style={{ padding: "16px 16px 0" }}>
            ...
            <StampCard stamps={c.stamps} settings={settings} />
    ...
    if (activeTab === "settings") return (
      <div style={{ padding: "16px 16px 0" }}>
        ...
        <div style={{ ...styles.card, marginBottom: 12 }}>
          ...
          <label style={styles.label}>Euro pro Punkt</label>
        ...
        <div style={{ ...styles.card, marginBottom: 12 }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "44px 1fr 44px", gap: 6, marginBottom: 6 }}>
              <IconPicker value={rewardForm.icon} onChange={v => setRewardForm(r => ({ ...r, icon: v }))} />
              <input placeholder="Name" value={rewardForm.name} onChange={e => setRewardForm(r => ({ ...r, name: e.target.value }))} style={{ ...styles.input, marginBottom: 0 }} />
              <input placeholder="#" type="number" value={rewardForm.stamps} onChange={e => setRewardForm(r => ({ ...r, stamps: e.target.value }))} style={{ ...styles.input, textAlign: "center", marginBottom: 0 }} />
            </div>
            <input placeholder="Beschreibung (optional)" value={rewardForm.description} onChange={e => setRewardForm(r => ({ ...r, description: e.target.value }))} style={{ ...styles.input, marginBottom: 6 }} />
            ...
            <button onClick={() => {
              ...
              setRewardForm({ stamps: "", name: "", icon: "fa-solid fa-id-card", description: "", keepAfterRedeem: false, pointsOnRedeem: 0 });
          ...
          {newAccResult ? (
            ...
            <>
              ...
              <button onClick={() => {
                ...
                const newC = { id: genId(), username: uname, password: pwd, firstLogin: true, name: "", birthdate: "", email: "", phone: "", points: 0, stamps: 0, memberSince: today(), tier: "bronze", language: "de", darkMode: true, redeemedBonuses: [], redeemedCoupons: [], history: [] };
